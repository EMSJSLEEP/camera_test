...
class VideoStreamApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.background()
        self.setWindowTitle("Video Stream Application")
        self.init_timer()
        self.init_log_file()
        ...
        if rpc_status:
            ...
            while(init_status):
                try:
                    ...
                    if self.control_camera['focus_setting'] != []:
                        init_status = 0
                    self.camera.set_exposure(self.pre_exposure)
                    self.camera.set_pwm_duty(5000, self.pre_brightness)
                    ...
                    if self.is_connect_success == False:
    ...
    def decode(self):
        ...
        if self.cap_now == False:
            if mix_8 == False:
                try:
                    data = self.camera.decode_online('AUTO', 'SAVE_BOTH', 3, 3000, timeout_ms=9000)
                except RPCError as rpc_error:
                     QMessageBox.information(self, "Error", 'decode time out')
                     return
            else:
                try:
                    data = self.camera.decode_online('AUTO', 'SAVE_BOTH', 3, 3000, rpc_timeout=9)
