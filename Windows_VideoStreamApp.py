...
import uuid
import sys
setattr(sys, 'OpenCV_LOADER_DEBUG', True)
import cv2
import os, time
import numpy as np
...
from windows_tool import Windows_Tools
...
class Windows_VideoStreamApp(QtWidgets.QMainWindow):
    def __init__(self):
        super(Windows_VideoStreamApp, self).__init__()
    ...
    def params_set(self):
        self.w = 640
        self.h = 480
        self.ex_value_limit = [-11, 3]
        self.fo_value_limit = [0, 1023]
        self.bytes_data = b''
        ...
        self.pixmap = None
        self.camera = None
        self.pre_exposure = -20
        self.pre_focus = -2
        self.pre_brightness = -3
    ...
    def connect(self):
        ...
        try:
            self.camera = cv2.VideoCapture(camera_num, cv2.CAP_DSHOW)
            # self.camera = cv2.VideoCapture(camera_num)
            self.w_tool = Windows_Tools(self.camera)
